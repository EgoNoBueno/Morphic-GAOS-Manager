...
class TestGetSecret:
    def test_returns_secret_value(self, mock_sm_client):
        mock_sm_client.access_secret_version.return_value.payload.data = (
            b"my-api-key-12345"
        )
        ...
        mock_sm_client.access_secret_version.assert_called_once_with(
            request={
                "name": "projects/morphic-gaos-prod/secrets/GEMINI_API_KEY/versions/latest"
            }
        )
    ...
    def test_raises_secret_access_denied(self, mock_sm_client):
        ...
        mock_sm_client.access_secret_version.side_effect = PermissionDenied(
            "permission denied"
        )
    ...
    def test_raises_secret_manager_error_on_unexpected(self, mock_sm_client):
        mock_sm_client.access_secret_version.side_effect = RuntimeError(
