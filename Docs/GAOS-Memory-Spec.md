...
├─────────────────────────────────────────────────────────────────┤
│  Layer 5b — Procedural Retrieval  Vertex AI Search              │
│  (semantic search over Layer 5 Drive corpus — read-only)        │
└─────────────────────────────────────────────────────────────────┘
```
...
| 5b — Retrieval | Vertex AI Search (Drive corpus index) | Automatic (Drive → Search sync) | All orchestrators (query at task time) | Mirrors Layer 5; updated on Drive file change |
...
> **Playbooks** (`playbooks/`) are added in Phase 2.5. A playbook is a structured project execution template generated by the Blueprint Factory when a `VISION_SUBMITTED` event is processed. Playbooks are not authored by humans — they are generated by Nexus-Prime and approved via the Approval Gate before being committed to Drive.

#### Playbook Document Schema

Playbooks live in `Knowledge/playbooks/` and follow this front-matter schema:

```yaml
---
title: "Q3 Product Launch — Morphic Widgets"
type: playbook
domain: nexus-prime
owner_agent: nexus-prime
version: 1
project_id: "proj_widgets_q3_2026"
created_from_vision: "User Chat submission 2026-03-16T18:00Z"
last_updated: "2026-03-16T18:05:00Z"
approved_by: "owner@company.com"
status: active                          # active | archived | draft
tags: [product-launch, q3, marketing, operations]
---
```

A playbook body contains: **Objective**, **Success Criteria**, **Agents Involved**, **Milestones** (dated list), **Constraints**, and **Open Questions**. Nexus-Prime writes the initial draft; the ITERATE_PLAN node appends and compacts constraints over the blueprint's lifecycle.

#### write_playbook Completion Requirement

Every domain orchestrator (Tier 2) must call `write_playbook` before marking a project-level task complete when:
- The task was initiated from a `VISION_SUBMITTED` event, **or**
- The task produced a reusable execution pattern that did not previously exist in `Knowledge/playbooks/`.

This requirement is enforced by Nexus-Prime at the `TASK_COMPLETE` handler — tasks that should have produced a playbook but did not are logged as `compliance_gap = TRUE` in BigQuery.
...
## 8. KNOWLEDGE_INJECTION Protocol (Scout Market Intelligence)

Scout's `_discover` recursive search node produces market intelligence observations that enter the self-learning loop via a dedicated fast-path message type: `KNOWLEDGE_INJECTION`.

**Governance rules:**

| Rule | Detail |
|------|--------|
| **Confidence gate** | Scout must corroborate a market intelligence finding across ≥ 5 independent web results before publishing `KNOWLEDGE_INJECTION`. The 5-observation gate matches the standard learning loop — no bypass. |
| **Knowledge type** | All Scout market intelligence is tagged `knowledge_type = "market_intel"` in the observation row. This allows Nexus-Prime to segregate domain knowledge from competitive intelligence in the Memory Bank. |
| **Approval required** | `knowledge_type = "market_intel"` observations follow the standard approval path — they are written to `Pending_Knowledge` and require owner approval before promotion to Layer 4. Scout may **not** self-promote market intelligence. |
| **Source citation required** | Every `KNOWLEDGE_INJECTION` payload must include `source_urls: list[str]` (≥ 1 URL) and `retrieval_date: str` (ISO 8601). Observations without citations are rejected by the `KNOWLEDGE_INJECTION` handler. |
| **Depth limit** | The `_discover` node is capped at 3 recursion levels to prevent runaway search costs. Each level may spawn ≤ 5 sub-queries. Maximum 15 total searches per `RESEARCH_MANDATE` invocation. |
| **project_id scoping** | `KNOWLEDGE_INJECTION` messages are scoped to the same `project_id` as the originating `RESEARCH_MANDATE`. Cross-project injection is forbidden. |

**Message flow:**

```
Nexus-Prime publishes RESEARCH_MANDATE → Scout
    ↓
Scout _plan node selects research targets
    ↓
Scout _discover node runs recursive web search (max depth 3)
    ↓
Confidence ≥ 0.70 across 5+ corroborated observations?
    ├── YES → Scout publishes KNOWLEDGE_INJECTION → Nexus-Prime
    │         Nexus-Prime writes to Pending_Knowledge (market_intel tag)
    │         Owner reviews → Approval Gate → Layer 4 promotion
    └── NO  → Observation stays in Scout working memory
              Flushed to Pending_Knowledge at session end (standard path)
```
