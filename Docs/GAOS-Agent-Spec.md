...
Every agent must accumulate `cost_usd` for each model call made during a task and return the total in `AgentOutput`. Gemini (`FAST_MODEL` and `DEEP_MODEL`) calls now return computed `cost_usd` values based on `prompt_token_count`, `candidates_token_count`, and the per-model pricing config in `settings.yaml` (`FAST_MODEL_INPUT_PRICE_PER_M`, `FAST_MODEL_OUTPUT_PRICE_PER_M`, `DEEP_MODEL_INPUT_PRICE_PER_M`, `DEEP_MODEL_OUTPUT_PRICE_PER_M`). On the AI Studio free tier, actual billing is $0 but the computed cost is tracked for capacity planning and displayed in the Grafana CEO dashboard. Ollama (`LOCAL_MODEL`) remains free; `cost_usd` is 0.0 for all Ollama calls. `tokens_used` is tracked for all models.
