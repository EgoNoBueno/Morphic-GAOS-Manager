...
- **Context Trio**: The three owner-context Markdown files — `Docs/about-me.md` (The Compass: business context and KPIs), `Docs/brand-voice.md` (The Persona: communication standards), and `Docs/working-preferences.md` (The Constitution: cost ceiling and workflow rules) — automatically appended to every agent's system prompt at boot via `_load_context_trio()` in `agents/__init__.py`. They ensure all seven orchestrators operate within the owner's business context without per-orchestrator configuration. Nexus-Prime's `think` node also uses the trio as the `system_prompt` for every reasoning call.
...
- **CX (Custom Search Engine ID)**: A Google identifier that scopes web searches to a configured set of domains or the open web, created via Google's Programmable Search Engine console. Stored in Secret Manager as `GOOGLE_SEARCH_CX` and used by `tools/google_search.py` alongside `GOOGLE_SEARCH_API_KEY` to give Scout bounded web search capability during Research Mandate tasks. The CX is paired with an API key and limited to 100 queries per day on the free tier.
...
- **MonologueFrame**: The structured pre-response reasoning record produced by Nexus-Prime's `think` node before every output-producing decision. Fields: `response_mode` (Direct / Research / Reframe / Tactical), `knowledge_gap_detected`, `knowledge_gap_description`, `partial_result_available`, and `reasoning_summary`. Written to BigQuery `aos_logs.monologue_frames` for observability and audit. When incoming message priority ≥ 4 (Tactical mode), the frame is constructed without an LLM call. Defined in `models/__init__.py`; documented in GAOS-Nexus-Prime-Spec.md §3.2.
...
- **`Project_Incubator`**: A Google Sheets tab that tracks Blueprint Docs from initial submission through owner approval. Each row records `blueprint_id`, vision text (truncated), `submitted_by`, `submitted_at`, `doc_id`, and `status`. Written by the `vision_blueprint` node in Nexus-Prime when a `VISION_SUBMITTED` message is processed. Rows remain pending until the owner approves or rejects the Chat card.
...
- **RESEARCH_MANDATE**: A Pub/Sub message type sent from Nexus-Prime to Scout to initiate deep structured web research on a specific topic. Scout's `_discover` node executes up to 15 Google Custom Search queries via `tools/google_search.py`, deduplicating results across queries. The output is returned as a `KNOWLEDGE_INJECTION` message when the confidence threshold (0.70 across ≥ 5 independent sources) is reached. Documented in GAOS-Agent-Spec.md.
...
- **SKILL_REQUEST**: A Pub/Sub message type published by a domain orchestrator when it encounters a `ModuleNotFoundError` on a library that is not in the import allowlist. Nexus-Prime's `handle_skill_request` node posts an approval card to the owner's Google Chat space, writes an audit row to `Agent_Approvals`, and either provisionally grants the import or issues an `ALERT` hard-stop based on the owner's response. The requested library must still pass the two-gate Static Analysis before any code using it can be deployed.
...
- **`Steward_Onboarding_Queue`**: A Google Sheets tab monitored by the Steward agent for new employee or stakeholder onboarding requests. Columns include `onboarding_id`, `full_name`, `role`, `email`, `department`, `start_date`, `access_tier`, `status`, `assigned_agent`, and `notes`. Rows are added manually by the owner or automatically by Steward when an onboarding event is detected. Access provisioning at tiers 3–5 requires an Approval Gate proposal; tiers 1–2 can be provisioned autonomously. Documented in GAOS-Onboarding-Spec.md §5.1.
...
- **TL;DR (Too Long; Didn't Read)**: Internet shorthand for a short summary — commonly placed at the beginning of a long post or document (though it can also appear at the end) — aimed at readers who want the bottom line without reading in full.
...
| **Google Custom Search API** | Bounded web search for Scout's Research Mandate tasks — scoped by a Custom Search Engine (CX) identifier; 100 free queries/day |
