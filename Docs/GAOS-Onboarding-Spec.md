...
5. Click **Create**.
6. Enable billing: **Billing → Link a billing account**. A credit card is required even though estimated monthly cost is ≈ $1.50. GCP will not charge you until you exceed the free tier.
...
**Cost:** Free to create; pay-as-you-go billing. See `GAOS-Manager-Spec.md §9.4` for the ≈ $1.50/month estimate.
...
**Cost:** Billed per token. See `GAOS-Manager-Spec.md §9.4` for estimates — typically $0.50–$0.60/month at normal AOS load.
...
def collect_config(state: dict) -> dict:
    ...
    info("  standard    — Gemini cloud LLM for complex reasoning (default, ≈$1.50/month)")
...
- [ ] BigQuery `aos_logs` dataset and 4 tables created with TTL partitioning
- [ ] `monologue_frames` table added (required by `GAOS-Persona-Spec.md §4`)
