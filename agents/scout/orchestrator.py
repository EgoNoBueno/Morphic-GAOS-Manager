...
def _park(state: AgentWorkingMemory) -> AgentWorkingMemory:
    ...
    proposal = ApprovalProposal(
        agent_id=_AGENT_ID,
        issue="Research result requires human approval",
...
def _route_after_boot(state: AgentWorkingMemory) -> str:
    ...
    msg_type = msg.get("message_type") if isinstance(msg, dict) else msg.message_type
...
def _initial_state(agent_input: AgentInput | Any) -> AgentWorkingMemory:
    ...
    if isinstance(agent_input, AgentInput):
        ...
        try:
            ...
            _log_cloud(
                ...
                "task",
                "",
                f"_initial_state: envelope decode failed: {exc}",
                "WARNING",
            )
