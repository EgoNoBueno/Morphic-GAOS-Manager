...
def _boot(state: AgentWorkingMemory) -> AgentWorkingMemory:
    ...
    try:
        ...
        pass
    try:
        state["memory_context"] = load_domain_memory(domain="operations", project_id=pid)
    except Exception:
        pass
    try:
        ...
        if r.get("status") == "stockout":
            try:
                ...
                publish(_OUTBOUND_TOPIC, A2AMessage(
                    ...
                    message_type=MessageType.ALERT, priority=4,
                    payload={"alert_type": "stock_insufficient", "sku": r.get("output", {}).get("sku", "")},
                ), state["project_id"])
            except Exception:
                pass
    return state
