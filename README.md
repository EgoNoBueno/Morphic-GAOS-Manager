...
**Plan Integrity (no TOCTOU).** The `apply` job downloads the binary plan generated by the `plan` job in the same workflow run. Apply executes exactly what was reviewed — a new commit pushed between plan and apply cannot silently alter what gets deployed.
